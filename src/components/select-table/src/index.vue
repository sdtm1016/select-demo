        ...
        >
          ...
          <template v-for="(item, index) in columns">
            <el-table-column
              ...
              v-bind="Object.assign({}, item.bind, $attrs)"
              v-on="$listeners"
            >
              <template slot-scope="scope">
                <!-- render方式 -->
...
export default {
        ...
        this.$nextTick(() => {
          ...
          if (this.multiple) {
            ...
            this.defaultValue = this.value
              ? { [this.keywords.label]: this.value }
              : ''
            // this.defaultValue = this.value
  ...
  methods: {
    ...
    filterMethodHandle(val) {
      console.log("filterMethodHandle,filterable:",this.filterable)
      console.log("filterMethodHandle,val:",val)
      ...
      if (tableData && tableData.length > 0) {
        this.tableData = tableData.filter((item) => {
          if (item[this.keywords.label].includes(val)) {
            return item
          }else if(PinyinMatch.match(item[this.keywords.label], val)) {
            return item
          }
        })
      }
    ...
    visibleChange(visible) {
      if (visible) {
        ...
        !this.filterMethod && this.filterMethodHandle('')
      }
    },
    ...
    findLabel() {
      this.$nextTick(() => {
        if (this.multiple) {
          ...
          if (this.$refs.select) {
            console.log('this.defaultValue---findLabel---this.defaultValue[this.keywords.label]', this.defaultValue[this.keywords.label])
